...
def test_cooldown_suppresses_duplicate():
    ...
    assert monitor.process_window(LOUD, now=101.0) is None
    assert monitor.process_window(LOUD, now=103.0) is not None
    assert len(events) == 2
...
def test_unmute_holds_until_buffer_refills():
    ...
    assert monitor.process_window(LOUD, now=101.5) is None
    assert monitor.process_window(LOUD, now=102.9) is None
    assert monitor.process_window(LOUD, now=103.1) is not None
    assert len(events) == 1
