...
# 9/2 실기에서 확인된 것: 로봇 자기 '네?' 에코가 첫 단어 지름길에서 승낙으로
# 읽혀 확인 없이 출발한 사례가 있다(미수리). 시험 중 갑작스런 출발에
# 대비하라고 대본에 적어 둔다. ('아니' 증발은 fb663d6 에서 수리됨.)
    ...
    for word in ("네", "응", "어", "그래", "아니", "대기해줘"):
        ...
        for i in range(5):
            steps.append({
                ...
                # 미션이 navigating 으로 남으면 다음 회차의 "화장실로 가자"가
                # "지금 이동 중입니다"로 막힌다 — 회차마다 idle 로 되돌린다.
                # 취소는 두 단계다(취소 요청 → "취소할까요?" → 긍정).
...
def _decline_steps() -> list:
    ...
    하나를 같이 잰다 — 뒤엣것이 없으면 '과하게 고쳤는지'를 알 수 없다."""
...
PLANS = {
    "호출": {
        ...
        "steps": _rep({
            ...
            # 창 안 관찰용 게이트가 gate_a 와 쿨다운을 나눠 쓰면 두 번째
            # 호출이 막힌다. 시험으로는 못박았지만 실기가 정본이다.
            "제목": "연달아 부르기 (쿨다운 회귀)",
            ...
            # 구제가 명령을 삼키면 안 된다 — 길이 조건이 지키는 경계.
    ...
    "에코": {
        ...
        "steps": _rep({
            ...
            # AND 를 켜면 겹치는 프레임이 3분의 1쯤 준다. 짧은 말로 끼어드는
            # 것이 가장 불리한 조건이라, 여기서 되면 나머지는 다 된다.
