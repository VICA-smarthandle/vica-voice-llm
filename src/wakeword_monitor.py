...
# 질문 답변 창의 정답 후보 (whisper 의 initial_prompt — hotwords 가 아니다).
# 기울이기일 뿐 강제가 아니라 자유 발화도 나올 수 있다.
#
# **여기 넣는 것은 "로봇이 물어서 받는 답"뿐이다.** 이 창을 여는 질문은
# 여섯 종류인데 다섯이 네/아니오이고, 숫자를 받는 것은 "몇 분쯤
# 걸리실까요?" 하나다. 종료·감사 표현은 사용자가 아무 때나 자유롭게 하는
# 말이지 질문의 답이 아니다.
#
# 2026-09-02 실측으로 그 여섯 개(이제 됐어·그만할래·안내 끝·다 됐어·
# 고마워·필요 없어요)를 뺐다. 하루치 로그에서 **오전사 11건이 전부
# 감사·종료 계열로 몰렸다** — whisper 가 짧고 애매한 소리를 이 문구들로
# 메꾼다: '다 됐어.' 2회('네'·'대기해줘'를 그렇게 적었다) · '다 됐어.
# 고마워.' · '고마워. 대기해.' · '고마워.' 2회('좋아'를 그렇게 적어
# 환각 필터에 삼켜졌다) · '고맙습니다' 3회 · '감사합니다.' 2회.
# 그중 '다 됐어' 2건은 finish(안내 전체 종료)로 갔다 — 가장 되돌리기
# 어려운 명령이다. 반대로 진짜 종료 의사를 놓친 사례는 0건이었다.
#
# 부수 효과: '고마워'·'다 됐어'가 여기서 빠지면 stt_guard 의 환각 목록과
# 겹치지 않는다. 같은 단어를 "이런 답이 나올 것"이라 알려주면서 나온
# 결과는 "유령이니 버려라"고 하던 모순이 사라진다.
CONFIRM_HINT = (
