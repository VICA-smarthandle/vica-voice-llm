...
구독: /vica/wake · /vica/user_text · /vica/intent · /vica/emergency
      /vica/tts_request (발화 요청) · /vica/tts_state (실제 재생 시작·종료)
      /vica/sim/event

TTS 는 요청과 재생을 나눠 잰다. /vica/intent 는 Mission Manager 전용 경로라
재생 시각의 근거가 되지 못한다 — 요청은 /vica/tts_request 로 들어오고 실제
소리는 /vica/tts_state 가 켜질 때 난다. 둘의 차이가 곧 큐 대기 시간이다.
...
class MetricsNode(Node):
    def __init__(self) -> None:
        ...
        self.create_subscription(String, "/vica/tts_request",
                                 lambda m: self._event("tts_request", m.data), 10)
