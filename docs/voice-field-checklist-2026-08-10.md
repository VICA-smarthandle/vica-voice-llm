...
**⚠️ "네"로 확정되는지 — 안 될 것으로 예상한다**

- [ ] "취소해줘" → 되묻기 → **"네"** → ________________ (되는지 적는다)

> 코드상 확정 경로는 `intent == "cancel"` 하나뿐이다(`mission_manager_node:267`).
> "네"는 목적지 확인 단축에도 안 걸린다 — 그 단축은 직전 AI 발화가 **목적지의**
> `confirm_prompt` 일 때만 걸리고, "안내를 취소할까요?"는 Mission Manager 가
> 말하므로 음성 쪽 대화 기록에 아예 없다.
>
> **안 되면 그것이 정상 동작이 아니라 고칠 거리다.** 사용자는 "네"라고 답한다.
> 결과를 적어 오면 다음 작업으로 잡는다.
